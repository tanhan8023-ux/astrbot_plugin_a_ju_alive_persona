"""
AstrBot 阿橘专属活人感插件 - 主入口

整合阿橘人设、情绪系统、记忆系统和随机行为，
...
logger = logging.getLogger("a_ju_alive_persona")
...
class AlivePersonaPlugin(Star):
    ...
    def __init__(self, context: Context, config: dict = None):
        ...
        # 好感度开关（可在 persona_a_ju.json 中关闭）
        self.enable_favorability = self.persona.persona.get('enable_favorability', True)

        logger.info(f"[阿橘] 已加载人设: {self.persona.get_name()}")
    ...
    async def on_llm_request(self, event: AstrMessageEvent, request):
        ...
        try:
            ...
            # 更新好感度（可通过 persona_a_ju.json 关闭）
            ...
            logger.exception("[阿橘] LLM 请求钩子失败，已跳过本轮活人感注入")
    ...
    async def on_llm_response(self, event: AstrMessageEvent, response):
        ...
        try:
            ...
            logger.exception("[阿橘] LLM 回复后处理失败，保留原始回复")
    ...
    def _build_reply_strategy(
        ...
        if mood in ('sleepy', 'bored', 'upset'):
            ...
            parts.append('心情不错，可以稍微活泼、好奇或黏人一点，但别突然变成话痨或疯狂堆喵')

        limit = self._reply_char_limit(style_decision.get('intent', 'casual') if style_decision else 'casual')
        parts.append(f'通常控制在{limit}字以内；不要连续追问，但自然的一句问话可以保留')
        return '。'.join(parts)
