...
class MemorySystem:
    ...
    def get_relation(self, user_id: str, is_special: bool = False) -> str:
        ...
        if fav >= 78 or (count >= 60 and fav >= 40):
            return 'close_friend'
        if fav >= 58 or (count >= 20 and fav >= 35):
            return 'friend'
        if fav >= 40 or count >= 4:
            return 'acquaintance'
        return 'stranger'
    ...
    def get_profile_description(self, user_id: str, special_prompt: str = None) -> str:
        ...
        rel_desc = {
            'close_friend': '你和ta关系很近，可以更黏人、更自然地分享小事',
            'friend': '你和ta已经熟悉，可以用轻微小傲娇和放松口吻',
            'acquaintance': '你和ta认识但还不够熟，友好回应并保留一点距离',
            'stranger': '你和ta还比较陌生，保持礼貌、简短和边界感',
        }
